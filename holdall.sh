...
readableDate(){ # convert seconds-since-epoch to human-readable
    set -euo pipefail
	local -r dateInSecondSinceEpoch=$1
...
generateBackupName(){
    set -euo pipefail
	local -r destLoc="$1"
	local -r backupName="$(dirname "$destLoc")/$(basename "$destLoc")-removed-$(date +%F-%H%M)~"
	echo "$backupName"
	return 0
}
...
modTimeOf(){
	...
	local -r queriedItem="$1"
	if [[ ! -e "$queriedItem" ]]; then echo "modTimeOf called on nonexistent file/folder $queriedItem - hard-coded mistake must exist...?"; exit 103; fi
...
addLocationToLocsList(){
	local -r locationToAddRaw="$1"
	local -r locationToAdd="$(readlink -m "$locationToAddRaw")"
...
listLocsListContents(){
	...
	local -r listOfItemNamesOnRmvbl="$(ls -B "$RMVBLDIR")"
	local -r listItemsNotSynced="$(comm -23 <(echo "$listOfItemNamesOnRmvbl") <(echo "$listOfItemNames"))"
	# comm -23 is returning things appearing in $listOfItemNamesOnRmvbl but not in $listOfItemNames
	local -r listItemsNotSyncedButAreSyncable="$(grep -v "^syncStatus$" <<< "$listItemsNotSynced" | grep -v "^syncLocationsOn_" | grep -v "^syncLog$")"
...
analyseLocsList(){
	...
	#| uniq -d \
	#)
	local -r listOfLocations="$(\
        cleanCommentsAndWhitespace "$LOCSLIST" \
        | grep -o "^[^|]*" \
        | grep -v "^\s*$" \
	)"
	local -r listOfRepeatedLocations="$(\
	...
	do
		...
		local noOfAppearances="$(sed -n -e "\:^$locationLine:p" <<< "$listOfLocations" | wc -l)" 
...
chooseVersionDialog(){ # ARGS 1)itemName 2)itemHostLoc 3)itemHostModTime 4)itemRmvblLoc 5)itemRmvblModTime 6)itemSyncTime
	local -r itemName="$1"
	local -r itemHostLoc="$2"
	local -r itemHostModTime=$3
	local -r itemHostModTimeReadable="$(readableDate "$itemHostModTime")" # the '@' indicates the date format as the number of seconds since the epoch
	local -r itemRmvblLoc="$4"
	local -r itemRmvblModTime=$5
	local -r itemRmvblModTimeReadable="$(readableDate "$itemRmvblModTime")"
	local -r itemSyncTime=$6
	local -r itemSyncTimeReadable="$(readableDate "$itemSyncTime")"
...
hostMissingDialog(){
	local -r itemName="$1"
	local -r itemRmvblLoc="$2"
	local -r itemHostLoc="$3"
	local -r itemHostLocRaw="$4"
...
synchronise(){ # caller should have ALREADY obtained permission with getPermission or user input, but the pretend mode check getPretend is handled in syncSourceToDest
	...
	case "$syncDirection" in
		...
		$DIRECTIONRMVBLTOHOST)
			...
			;;
		*)
			echo "synchronise was passed invalid argument $syncDirection, there must be a hard-coded fault..."
...
syncSourceToDest(){
	...
	then 
		...
		local -r backupName="$(generateBackupName "$destLoc")"
		shortOpts="$shortOpts"b 
		local -r longOptBackup="--backup-dir=$backupName" 
	else
...
removeOldBackups(){
	local -r locationStem="$1"
	
	getVerbose && echo "checking for expired backups"
	local -r existingBackupsSorted="$(ls -d "$locationStem"-removed* | grep '^'"$locationStem"'-removed-2[0-9][0-9][0-9]-[01][0-9]-[0-3][0-9]-[0-2][0-9][0-5][0-9]~$' | sort)" # this can return an empty string
	getVerbose && echo -e "backups existing:\n$existingBackupsSorted"
	local -r oldBackups="$(head --lines=-$NOOFBACKUPSTOKEEP <<<"$existingBackupsSorted")"
...
writeToStatusFileLASTSYNCDATEnow(){
	...
	local -r timeStampToSet="$(( $(date +%s) ))" # i.e. the current time
